...
macro_rules
  | `(tactic| absurdHyp $h:ident ($n:ident)) =>
      `(tactic| rewrite [($n)] at ($h); have z: False := ($h); cases z)
  | `(tactic| absurdHyp $h:ident ($n:ident, $m:ident)) =>
      `(tactic| rewrite [($n), ($m)] at ($h); simp at ($h); exact False.elim (($h) True.intro))
...
/-            split at h'' -/
/-            case h_1 f k₁ k₂ h₁ h₂ => -/
/-              have kk' : (fun Γ Δ => ¬ k' Γ Δ) = k := -/
/-                by injection h' with _ z; exact z -/
/-              have k'k₁k₂ : (λ Γ Δ => ((k₁ Γ Δ) → (k₂ Γ Δ))) = k' := -/
/-                by injection h'' with _ z; exact z -/
/-              rewrite [← k'k₁k₂] at kk' -/
/-              simp -/
/-              rewrite [h₁] -/
...
/-              intro notk₁ -/
/-              rewrite [← kk'] at h -/
/-              exact h (λ k₁ => False.elim (notk₁ k₁)) -/
/-            case h_2 x'' f h3 => -/
/-              have k'T : (λ _ _ => True) = k' := -/
/-                by injection h'' with _ z; exact z -/
/-              rewrite [← k'T] at h' -/
/-              have kF : (λ Γ Δ => ¬ (λ _ _ => True) Γ Δ) = k := -/
/-                by injection h' with _ z; exact z -/
/-              rewrite [← kF] at h -/
  ...
  followsFrom (not $ implies t₁ t₂) t₁ :=
    by intros t₁ t₂ Γ Δ h
       ...
       | ⟨ atom 1, k₁ ⟩, ⟨ atom 1, k₂ ⟩ =>
         ...
         apply byContradiction
         intro notk₁
         exact False.elim (h' (λ k₁ => False.elim (notk₁ k₁)))
       | ⟨ atom 0, _ ⟩, _  => simp
       | ⟨ atom 1, k₁ ⟩, ⟨ atom 0, _ ⟩  => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ atom (succ (succ _)), _ ⟩ => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ sort.undef, _ ⟩           => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ sort.array _ _, _ ⟩       => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ sort.bv _, _ ⟩            => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ sort.arrow _ _, _ ⟩       => absurdHyp h (r₁, r₂)
       | ⟨ atom 1, k₁ ⟩, ⟨ sort.dep, _ ⟩             => absurdHyp h (r₁, r₂)
       | ⟨ atom (succ (succ _)), _ ⟩, _ => simp
       | ⟨ sort.undef, _ ⟩, _           => simp
       | ⟨ sort.array _ _, _ ⟩, _       => simp
       | ⟨ sort.bv _, _ ⟩, _            => simp
       | ⟨ sort.arrow _ _, _ ⟩, _       => simp
       | ⟨ sort.dep, _ ⟩, _             => simp
  ...
  followsFrom (not $ implies t₁ t₂) (not t₂) :=
    by intros t₁ t₂ Γ Δ h
       ...
       | ⟨ atom 1, k₁ ⟩, ⟨ atom 1, k₂ ⟩ =>
           ...
           | Or.inr r  => exact r
       | _, ⟨ atom 0, _ ⟩               => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ atom (succ (succ _)), _ ⟩ => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ sort.undef, _ ⟩           => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ sort.array _ _, _ ⟩       => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ sort.dep, _ ⟩             => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ sort.bv _, _ ⟩            => simp; rewrite [r₂]; exact True.intro
       | _, ⟨ sort.arrow _ _, _ ⟩       => simp; rewrite [r₂]; exact True.intro
       | ⟨ atom 0, _ ⟩, ⟨ atom 1, k₂ ⟩               => absurdHyp h (r₁, r₂)
       | ⟨ atom (succ (succ _)), _ ⟩, ⟨ atom 1, k₂ ⟩ => absurdHyp h (r₁, r₂)
       | ⟨ sort.undef, _ ⟩, ⟨ atom 1, k₂ ⟩           => absurdHyp h (r₁, r₂)
       | ⟨ sort.array _ _, _ ⟩, ⟨ atom 1, k₂ ⟩       => absurdHyp h (r₁, r₂)
       | ⟨ sort.dep, _ ⟩, ⟨ atom 1, k₂ ⟩             => absurdHyp h (r₁, r₂)
       | ⟨ sort.bv _, _ ⟩, ⟨ atom 1, k₂ ⟩            => absurdHyp h (r₁, r₂)
       | ⟨ sort.arrow _ _, _ ⟩, ⟨ atom 1, k₂ ⟩       => absurdHyp h (r₁, r₂)


/- theorem orIntro : ∀ {t₁ t₂ : term}, -/
/-   followsFrom t₁ (or t₂ t₁) -/
/-   | t₁, t₂, Γ, Δ, h => -/
/-     by simp at h -/
/-        simp -/

-- t1 mal formado e t2 falso? -> absurdHyp
