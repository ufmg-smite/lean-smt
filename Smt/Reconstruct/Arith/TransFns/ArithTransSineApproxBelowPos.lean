...
theorem arithTransSineApproxBelowPos (d n : ℕ) (_ : d = 2 * n) (t lb ub l u : ℝ) (ht : lb ≤ t ∧ t ≤ ub) :
...
/-     Real.exp x ≥ taylorWithinEval Real.exp d Set.univ 0 x := by -/
/-   rw [hx] -/
/-   simp only [exp_zero, taylorWithinEval_self, ge_iff_le, le_refl] -/

/- theorem arithTransExpApproxBelow (d n : ℕ) (h : d = 2*n + 1) : -/
/-     Real.exp x ≥ taylorWithinEval Real.exp d Set.univ 0 x := by -/
/-   cases lt_trichotomy 0 x with -/
/-   | inl hlt => exact arithTransExpApproxBelow₁ d n h hlt -/
/-   | inr hge => -/
/-     cases hge with -/
/-     | inl heq => exact arithTransExpApproxBelow₃ d n h heq -/
/-     | inr hgt => exact arithTransExpApproxBelow₂ d n h hgt -/
