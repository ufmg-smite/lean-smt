...
theorem le_of_not_le {a b : Rat} : ¬ a ≤ b → b ≤ a := Rat.le_total.resolve_left
...
private theorem Rat.mul_le_mul_left {c x y : Rat} (hc : c > 0) : (c * x ≤ c * y) = (x ≤ y) := by
  ext; constructor
  · apply Rat.le_of_mul_le_mul_left (hc := hc)
  · apply Rat.mul_le_mul_of_nonneg_left (hc := Rat.le_of_lt hc)

private theorem Rat.mul_le_mul_left' {c x y : Rat} (hc : c ≥ 0) : x ≤ y → (c * x ≤ c * y) := by
  intro h
  cases Rat.le_iff_lt_or_eq.mp hc
  next hlt =>
...
private theorem Rat.mul_eq_zero_left {x y : Rat} : x ≠ 0 → x * y = 0 → y = 0 :=
  numDenCasesOn' x fun nx dx nz_dx =>
    numDenCasesOn' y fun ny dy nz_dy => by
       ...
       rw [Rat.divInt_mul_divInt nx ny] at h2
...
theorem abs_eq {a b : Rat} (hb : 0 ≤ b) : a.abs = b ↔ a = b ∨ a = -b := by
  ...
  next hr =>
    ...
    rw [h] at this
    have : 0 = b := Eq.symm (Rat.nonneg_antisymm hb this)
    rw [<- this] at h
    simp at h
    exact trans h this
...
theorem neg_of_pos {a : Rat} : 0 < a → -a < 0 := by
  ...
  have : -a = -a + 0 := by simp [Rat.add_zero]
  conv =>
    lhs
    rw [this]
    skip
...
theorem eq_eq_sub_eq_zero : (a = b) = (a - b = 0) := by
  apply propext
  constructor
  · intro h; rw [h]; simp [Rat.sub_self]
  · intro h
...
theorem le_mul_of_lt_of_le (a b : Rat) : a < 0 → b ≤ 0 → 0 ≤ a * b := by
  intros h1 h2
  cases Rat.le_iff_eq_or_lt.mp h2 with
  | inl hlt => have := mul_sign₁ h1 hlt; exact le_of_lt this
  | inr heq => rw [heq, Rat.mul_zero]; exact rfl
...
theorem neg_of_mul_pos (a b : Rat) : a < 0 → 0 < a * b → b < 0 := by
  ...
  cases Rat.le_iff_eq_or_lt.mp this with
  | inl hlt => have := mul_sign₃ h1 hlt; have := Rat.lt_trans h2 this; exact Rat.lt_irrefl this
  | inr heq => rw [<-heq, Rat.mul_zero] at h2; exact Rat.lt_irrefl h2
...
theorem eq_of_sub_eq {c₁ c₂ : Rat} (hc₁ : c₁ ≠ 0) (hc₂ : c₂ ≠ 0) (h : c₁ * (a₁ - a₂) = c₂ * (b₁ - b₂)) : (a₁ = a₂) = (b₁ = b₂) := by
  ...
  · intro ha
    rw [ha] at h
    simp only [Rat.sub_self, Rat.mul_zero] at h
...
theorem ge_of_sub_eq_neg {c₁ c₂ : Rat} (hc₁ : c₁ < 0) (hc₂ : c₂ < 0) (h : c₁ * (a₁ - a₂) = c₂ * (b₁ - b₂)) : (a₁ ≥ a₂) = (b₁ ≥ b₂) := by
  show (a₂ ≤ a₁) = (b₂ ≤ b₁)
  rw [<- Rat.neg_sub, <- Rat.neg_sub b₂ b₁, mul_neg, mul_neg] at h
  have h' := neg_eq h
  exact le_of_sub_eq_neg hc₁ hc₂ h'
...
theorem mul_tangent_mp_lower (h : x * y ≤ b * x + a * y - a * b)
  ...
  | inr h2 =>
    ...
    · apply Classical.byContradiction
      ...
      exact Rat.lt_irrefl (Rat.lt_of_le_of_lt h h3)
...
theorem mul_tangent_mpr_lower (h : x ≤ a ∧ y ≥ b ∨ x ≥ a ∧ y ≤ b)
  ...
  | inl h =>
    ...
    cases Rat.le_iff_eq_or_lt.mp this with
    | inl lt => rw [Rat.mul_le_mul_left lt]; exact h1
    | inr eq => rw [<- eq]; simp only [Rat.zero_mul, ge_iff_le]; rfl
  | inr h =>
    have ⟨h1, h2⟩ := h
    have : y - b ≤ 0 := (le_iff_sub_nonneg' b y).mp h2
    cases Rat.le_iff_eq_or_lt.mp this with
    | inl lt => show (y - b) * a ≥ (y - b) * x; rw [<- Rat.mul_le_mul_of_neg_left a x _ lt]; exact h1
    | inr eq => rw [eq]; simp only [Rat.zero_mul, ge_iff_le]; rfl
...
theorem mul_tangent_mp_upper (h : x * y ≥ b * x + a * y - a * b)
  ...
  | inl h2 =>
    ...
    · apply Classical.byContradiction
      ...
      rw [Rat.mul_le_mul_left this] at h
      exact Rat.lt_irrefl (Rat.lt_of_lt_of_le h2 h)
  | inr h2 =>
    constructor
...
theorem mul_tangent_mpr_upper (h : x ≤ a ∧ y ≤ b ∨ x ≥ a ∧ y ≥ b)
  ...
  | inl h =>
    ...
    | inr eq => rw [eq]; simp only [Rat.sub_self, Rat.zero_mul, ge_iff_le]; rfl
  | inr h =>
    have ⟨h1, h2⟩ := h
    cases Rat.le_iff_eq_or_lt.mp h2 with
    | inl lt =>
