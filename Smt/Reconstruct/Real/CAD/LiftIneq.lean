...
def lift_ineq (ineq_pf : Expr) : MetaM (Expr × Expr) := do
  ...
  let some (var : Q(Real)) ← get_var monoms | throwError "get_var failed" -- TODO: this should come from cvc5
...
syntax (name := lift_ineq_tac) "lift_ineq" term : tactic
